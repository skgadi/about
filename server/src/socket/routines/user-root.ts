...
import { GSK_CS_USER_ROOT_PIC } from "../../services/library/types/data-transfer/user-root.js";
...
export const routines = (io: any, socket: any) => {
  socket.on("GSK_CS_USER_ROOT_PIC", async (data: GSK_CS_USER_ROOT_PIC) => {
    try {
      ...
      console.log(`User ${userId} root picture updated successfully.`);
    } catch (error) {
      notifyErrorToClient(
        socket,
        "User Root PIC Error",
      ...
      console.error("Error handling GSK_CS_USER_ROOT_PIC:", error);
    }
  });
};
